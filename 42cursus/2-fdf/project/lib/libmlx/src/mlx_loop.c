...
/*   Updated: 2023/01/19 12:28:42 by kichkiro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mlx_loop(t_xvar *xvar)
{
	...
	while (win_count(xvar) && !xvar->end_loop)
	{
		while (!xvar->end_loop && (!xvar->loop_hook || XPending(xvar->display)))
		{
			...
			if (win && ev.type == ClientMessage && ev.xclient.message_type == \
				xvar->wm_protocols && ev.xclient.data.l[0] == \
				xvar->wm_delete_window && win->hooks[DestroyNotify].hook)
