...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2000/12/08 11:07:24 by CharlieRoot       #+#    #+#             */
/*   Updated: 2023/01/19 11:24:25 by kichkiro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mlx_int_egal_img(XImage *img1, XImage *img2)
{
	if (img1->width != img2->width || img1->height != img2->height || \
		...
		img1->byte_order != img2->byte_order || img1->bitmap_unit != \
		img2->bitmap_unit || img1->bitmap_bit_order != img2->bitmap_bit_order \
		|| img1->bitmap_pad != img2->bitmap_pad || img1->depth != img2->depth \
		|| img1->bytes_per_line != img2->bytes_per_line || \
		img1->bits_per_pixel != img2->bits_per_pixel || img1->red_mask != \
		img2->red_mask || img1->green_mask != img2->green_mask || \
...
void	*mlx_xpm_file_to_image(t_xvar *xvar, char *filename, int *width, \
	int *height)
{
	return (mlx_int_xpm_f_image(xvar, width, height, XpmReadFileToImage, \
		filename));
}
