...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2001/12/11 15:25:27 by OlivierCrou       #+#    #+#             */
/*   Updated: 2023/01/19 12:24:57 by kichkiro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#define	RETURN	{ if (colors) free(colors); if (tab) free(tab); \
	tab = (void *)0; if (colors_direct) free(colors_direct); if (img) \
	{XDestroyImage(img->image); XFreePixmap(xvar->display,img->pix); \
	free(img);} return ((void *)0); }
	...
	char *(*f)())
{
	...
	nc = atoi(tab[2]);
	cpp = atoi(tab[3]);
	if (!line || !tab || !width || !height || !nc || !cpp)
		RETURN ;
	free(tab);
	tab = 0;
...
int	mlx_int_file_get_rid_comment(char *ptr, int size)
{
	...
	while (com_begin != -1)
	{
		com_end = mlx_int_str_str(ptr + com_begin + 2, "*/", size - com_begin \
			- 2);
